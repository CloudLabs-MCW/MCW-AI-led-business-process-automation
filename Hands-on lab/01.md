...
4. Go back to your resource group and select **contoso-fr-SUFFIX (1)** Azure Forms Recognizer service where SUFFIX represents a unique string specific to your resource group.
   ...
   ![Lab resource group is open. Azure Forms Recognizer resource is highlighted.](media/contoso-form-recognizer.png "Select Forms Recognizer Service")
...
### Task 3: Implementing Forms Recognizer for document processing (Read-Only)

  >**Note**: This task is for understanding the concept of Implementing Forms Recognizer for document processing through Visual studio which is already deployed.
...
2. From the left menu, select **Access keys (1)**. Select **Show keys (2)** to reveal the keys. Select the copy button **(3)** next to the **key1** connection string and paste it to a text editor of your choice. This value will be used later in the lab.
   ...
   ![Storage Account Access keys page is shown. The show Keys button is selected. The copy button for the first connection string is highlighted.](media/contoso-storage-accesskeys.png "Copy Storage Connection String")
...
1. Go back Azure Portal and Navigate to **contosoSUFFIX** storage account in your lab resource group.

    ![Edge is highlighted on the desktop. Browser is open and navigated to portal.azure.com. Storage account overview page is open.](media/ex1-storageaccount.png "Storage Account on Lab VM")

2. From the left menu, select **Containers (1)**, then select the **claims (2)** container.
...
3. Select **Upload (1)** and **Browse (2)**. Navigate to `C:\MCW\MCW-main\Hands-on lab\lab-files\claims-forms` **(3)**. Pick **20210621-test-form (4)** and select **Open (5)**. This PDF file was not used during model training.
...
5. Our Azure Function for claims form processing has triggered with the addition of this file in storage. You should see the result in the Cosmos DB service. Go back to your resource group and select your Cosmos DB resource named `contoso-cdb-SUFFIX` from your lab resource group.
...
7. Select the **Items (1)** list under the **Contoso** database's **Claims** collection. Select the first document **(2)** to see its content. Take a look at the values extracted by Forms Recognizer, such as **PatientName** and **Diagnosis (3)**.
...
8. Repeat steps 10 to 14 to upload the remaining PDF files in the `C:\MCW\MCW-main\Hands-on lab\lab-files\claims-forms` folder. You can upload multiple files at one time.
